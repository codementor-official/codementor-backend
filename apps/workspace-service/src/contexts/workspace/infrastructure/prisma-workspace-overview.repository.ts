...
export class PrismaWorkspaceOverviewRepository implements WorkspaceOverviewRepository {
  ...
  async get(
          ...
          include: {
            users: {
              select: {
                ...
                email: true,
                avatar_url: true,
                user_stats: { select: { xp: true, solved_count: true, current_streak_days: true } },
              },
            },
          },
        }),
    ...
    const documentCount = (status: document_status) =>
      documents.find((item) => item.status === status)?._count._all ?? 0;
    const completedAssignment = (status: assignment_status) =>
      status === assignment_status.done || status === assignment_status.late;
    const assignmentCompleted = assignments.filter((item) =>
      completedAssignment(item.status),
    ).length;
    const assignmentLate = assignments.filter(
      (item) => item.status === assignment_status.late,
    ).length;
    const assignmentInProgress = assignments.filter(
      (item) => item.status === assignment_status.inprogress,
    ).length;
    const assignmentNotStarted = assignments.filter(
      (item) => item.status === assignment_status.notstarted,
    ).length;
    const accepted = submissions.filter((item) => item.verdict === submission_verdict.accepted);
    const failed = submissions.filter(
      (item) =>
        item.verdict !== submission_verdict.accepted && item.verdict !== submission_verdict.pending,
    );
    ...
    submissions.forEach((item) => {
      if (item.assignment_id)
        attemptsByAssignment.set(
          item.assignment_id,
          (attemptsByAssignment.get(item.assignment_id) ?? 0) + 1,
        );
    });
    const attemptCounts = [...attemptsByAssignment.values()];
    const now = new Date();
    const submissionTrend = trend(
      14,
      (day, nextDay) =>
        submissions.filter((row) => row.submitted_at >= day && row.submitted_at < nextDay).length,
    );
    const completionTrend = trend(
      14,
      (day, nextDay) =>
        accepted.filter((row) => row.submitted_at >= day && row.submitted_at < nextDay).length,
    );
    const activityTrend = trend(
      28,
      (day, nextDay) =>
        recentActivities.filter((row) => row.created_at >= day && row.created_at < nextDay).length,
    );
    ...
    const overviewMembers = members.map((member) => {
      ...
      const completedCount = memberAssignments.filter((assignment) =>
        completedAssignment(assignment.status),
      ).length;
      const memberSubmissions = submissions.filter(
        (submission) => submission.user_id === member.user_id,
      );
      const memberScores = memberSubmissions.flatMap((submission) =>
        submission.score === null ? [] : [submission.score],
      );
      const memberActivities = recentActivities.filter(
        (activity) => activity.actor_id === member.user_id,
      );
      const lastActiveAt =
        [
          ...memberActivities.map((activity) => activity.created_at),
          ...memberSubmissions.map((submission) => submission.submitted_at),
      ...
      return {
        ...
        xp: member.users.user_stats?.xp ?? 0,
        solvedCount: member.users.user_stats?.solved_count ?? 0,
        streakDays: member.users.user_stats?.current_streak_days ?? 0,
        ...
        acceptedCount: memberSubmissions.filter(
          (submission) => submission.verdict === submission_verdict.accepted,
        ).length,
    ...
    const progressDistribution = [
      {
        ...
        value: learners.filter((item) => item.completionRate === 100).length,
      },
      {
        key: 'advanced',
        label: '70–99%',
        value: learners.filter((item) => item.completionRate >= 70 && item.completionRate < 100)
          .length,
      },
      {
        key: 'steady',
        label: '40–69%',
        value: learners.filter((item) => item.completionRate >= 40 && item.completionRate < 70)
          .length,
      },
      {
        key: 'starting',
        label: '1–39%',
        value: learners.filter((item) => item.completionRate > 0 && item.completionRate < 40)
          .length,
      },
      {
        key: 'not_started',
        label: 'Chưa bắt đầu',
        value: learners.filter((item) => item.completionRate === 0).length,
      },
    ];
    return {
      documents: {
        total: documents.reduce((total, item) => total + item._count._all, 0),
        published: documentCount(document_status.published),
        pending: documentCount(document_status.pending),
      },
      exercises: {
        total: exercises.length,
        open: exercises.filter(
          (item) =>
            item.exercises.status === exercise_status.published &&
            (!item.due_at || item.due_at >= now),
        ).length,
      },
      assignments: {
        ...
        notStarted: assignmentNotStarted,
        late: assignmentLate,
        completionRate:
          assignments.length === 0
            ? 0
            : Math.round((assignmentCompleted / assignments.length) * 100),
      },
      submissions: {
        ...
        acceptanceRate:
          submissions.length === 0 ? 0 : Math.round((accepted.length / submissions.length) * 100),
        averageScore: average(scores),
        averageAttempts: average(attemptCounts),
        maxAttempts: attemptCounts.length ? Math.max(...attemptCounts) : 0,
      },
      members: overviewMembers,
      activities: activities.map((activity) => ({
        id: activity.id,
        actor: activity.users?.display_name ?? null,
...
function trend(days: number, count: (day: Date, nextDay: Date) => number) {
  return Array.from({ length: days }, (_, index) => {
    const day = startOfUtcDay(-(days - 1 - index));
    const nextDay = new Date(day);
    nextDay.setUTCDate(nextDay.getUTCDate() + 1);
    return {
      label: new Intl.DateTimeFormat('vi-VN', { day: '2-digit', month: '2-digit' }).format(day),
      value: count(day, nextDay),
    };
  });
}

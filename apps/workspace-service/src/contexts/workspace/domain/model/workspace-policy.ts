...
export const WORKSPACE_PERMISSIONS = [
  'upload_doc', 'create_exercise', 'edit_exercise', 'delete_doc', 'review_submission', 'remove_member',
] as const;
...
export type PermissionGrant = { role: WorkspaceRole; permission: WorkspacePermission; allowed: boolean };
...
export const DEFAULT_ROLE_PERMISSIONS: Record<ConfigurableWorkspaceRole, Partial<EffectivePermissions>> = {
  deputy: { upload_doc: true, create_exercise: true, edit_exercise: true, review_submission: true, remove_member: true },
...
export function resolveWorkspacePermissions(
  ...
  const result = Object.fromEntries(WORKSPACE_PERMISSIONS.map((permission) => [permission, role === 'owner'])) as EffectivePermissions;
  if (role === 'owner') return result;
  for (const grant of rolePermissions) if (grant.role === role) result[grant.permission] = grant.allowed;
  for (const override of overrides) result[override.permission] = override.allowed;
  return result;
}

export function rolePermissionsFor(role: ConfigurableWorkspaceRole, grants: PermissionGrant[]): EffectivePermissions {
...
export function normalisePermissionPatch(input: Record<string, boolean>): { permission: WorkspacePermission; allowed: boolean }[] {
  const unknown = Object.keys(input).find((permission) => !WORKSPACE_PERMISSIONS.includes(permission as WorkspacePermission));
  if (unknown) throw new BusinessRuleViolation(`Quyền không hợp lệ: ${unknown}`);
  return Object.entries(input).map(([permission, allowed]) => ({ permission: permission as WorkspacePermission, allowed: Boolean(allowed) }));
}
